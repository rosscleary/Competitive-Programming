...
string back_direct (char direction)
...

{
  ...
  vector<pair<char,string> > directions;
  ...
  while (1)
  {
    ...
    cin >> direction >> street;

    directions.pb(mp(direction, street));
  ...
  for (int i = directions.sz() - 1; i > 0; i--)
  {
    cout << "Turn " << back_direct(directions[i].f) << " onto " << directions[i - 1].s << " street." << endl;
  }

  cout << "Turn " << back_direct(directions[0].f) << " into your HOME." << endl;
