...
int N, M, D; Ve<In> day_event_count;
int answer;
...
RI main() { fast_inputandoutput
  ...
  while(test_case--) {
    cin >> N >> M >> D;
    day_event_count.Cl(); day_event_count.RS(D + 1);
    for(int i = 0; i < M; i++) {
      int A_i; cin >> A_i;
      day_event_count[A_i]++;
    }

    int clean_amount = N, total_amount = N;
    answer = 0;
    for(int day = 1; day <= D; day++) {
      ...
      clean_amount += day_event_count[day];
      total_amount += day_event_count[day];
      clean_amount--;
    }
