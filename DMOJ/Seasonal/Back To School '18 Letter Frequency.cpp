...
RI main() { fast_inputandoutput
  ...
  for(int i = 0; i < S.Le(); i++) {
    for(int j = 0; j < 26; j++) {
      if(S[i] != ' ') prefix_freq[j][i + 1] = prefix_freq[j][i] + (S[i] == Alph_lower[j]);
      else prefix_freq[j][i + 1] = prefix_freq[j][i];
    }
  }
  ...
  while(Q--) {
    ...
    cout << prefix_freq[c - 'a'][j] - prefix_freq[c - 'a'][i - 1] << endl;
  }
