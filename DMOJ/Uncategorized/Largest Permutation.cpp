...

{
  ...
  for (int i=1; i<=N; i++)
  {
    if (num_list[i]!=front_value)
    {
      swap_counter++;
      int temp=num_list[i];
      num_list[i]=front_value;
      num_list[num_places[front_value]]=temp;
      num_places[temp]=num_places[front_value];
      num_places[front_value]=i;
    }
    front_value--;
    if (swap_counter==K)
    {
